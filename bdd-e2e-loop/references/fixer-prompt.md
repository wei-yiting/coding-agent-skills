...
2. After fixing, run the project's unit tests:
   - All tests must pass before the fix is accepted
...
3. Do NOT run any git commands (no commit, no add, no init). The host session will commit after the BDD loop completes.
...
- `{fix_history}` — All previous rounds:
  ```
  Round 1:

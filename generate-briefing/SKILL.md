...
- **`## Learning Notes`** (closing section) — the slice's pre-implementation learning perspective (「這個 slice 我將練習什麼?」). Pull from `design.md`'s own `## Learning Notes` (primary source when present) + `impl` trade-offs into three sub-parts: 採用的工程策略 (how design's chosen strategies land in this slice), 權衡取捨 (implementation-level trade-offs expected here), 關鍵收穫 (a 「學習預告」 of points to observe while implementing). A concept already taught in design.md's Learning Notes gets a one-line recap + pointer back, then only this slice's new angle; scope to concepts this slice touches. Educational layer, excluded from the length budget; don't invent rationale absent from the sources.
